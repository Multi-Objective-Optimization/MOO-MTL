# Hessian-Vector Product solvers.
...
class HVPSolver(object):
    """Base class for Hessian-Vector Product computation."""
    ...
    def set_hess(self, *, batch: bool = True, num_batches: int = None) -> None:
        self.apply = self.apply_batch if batch else partial(self.apply_full, num_batches=num_batches)

    def set_grad(self, *, batch: bool = True, num_batches: int = None) -> None:
    ...
    @torch.enable_grad()
    def apply_full(self, vec: Tensor, weights: Tensor = None, *, grads: Tensor = None, num_batches: int = None, retain_graph: bool = False) -> Tensor:
        num_batches = len(self.dataloader) if num_batches is None else num_batches
        weighted_hvp = None
        for _ in range(num_batches):
            weighted_hvp_batch, _ = self.apply_batch(vec, weights, grads=grads, retain_graph=retain_graph)
    ...
    @torch.enable_grad()
    def grad_full(self, *, create_graph: bool = False, num_batches: int = None) -> Tensor:
        num_batches = len(self.dataloader) if num_batches is None else num_batches
        grads = None
        for _ in range(num_batches):
        ...
        return grads.clone().detach()


class AutogradHVPSolver(HVPSolver):
    """HVP solver using PyTorch autograd."""
    ...
    def apply_batch(self, vec: Tensor, weights: Tensor = None, *, grads: Tensor = None, retain_graph: bool = True) -> Tuple[Tensor, Tensor]:
        ...
        weighted_grad = grads.sum(dim=0) if weights is None else torch.matmul(weights, grads)
...
class VisionHVPSolver(AutogradHVPSolver):
    """HVP solver for vision networks with closure-based loss computation."""
