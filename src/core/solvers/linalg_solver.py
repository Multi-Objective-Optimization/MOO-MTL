# Linear algebra solvers (MINRES, CG) for KKT system.
...
class HVPLinearOperator(LinearOperator):
    ...
    def __init__(self, network: nn.Module, hvp_solver: HVPSolver, device: torch.device, damping: float) -> None:
    ...
    def _matvec_tensor(self, tensor: Tensor) -> Tensor:
        alphas_hvps, _ = self.hvp_solver.apply(
            tensor, self.alphas, grads=self.jacobians, retain_graph=self.jacobians is not None)
        if self.damping > 0.0:
    ...
    def _matvec(self, x: np.ndarray) -> np.ndarray:
        ...
        return self._matvec_tensor(tensor).detach().cpu().numpy()


class KrylovSolver(object):

    def solve(self, lazy_jacobians: Tensor, jacobians: Tensor, alphas: Tensor, rhs: Tensor, *, verbose: bool = False):
        raise NotImplementedError


class MINRESSolver(KrylovSolver):

    def __init__(self, network: nn.Module, hvp_solver: HVPSolver, device: torch.device, shift: float, tol: float, damping: float, maxiter: int) -> None:
        self.device = device
    ...
    def solve(self, lazy_jacobians: Tensor, jacobians: Tensor, alphas: Tensor, rhs: Tensor, *, verbose: bool = False):
        try:
            ...
            results = self.minres(self.linear_operator, rhs_np, show=verbose, x0=x0)
...
class CGSolver(KrylovSolver):
    ...
    def __init__(self, hvp_solver: HVPSolver, device: torch.device, tol: float, damping: float, maxiter: int, pd_strict: bool = False) -> None:
        self.hvp_solver = hvp_solver
        self.device = device
        self.tol = tol
        self.damping = damping
    ...
    def cg(self, lazy_jacobians: Tensor, alphas: Tensor, rhs: Tensor, x0: Tensor = None, *, verbose: bool = False) -> Tensor:
